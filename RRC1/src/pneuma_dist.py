...
# No scaling of the length matrix
# **Scale the length matrix**
# length_scale = np.mean(length_matrix.data)
# length_matrix = length_matrix / length_scale

# Step 1: Extract the non-zero elements (data) from the sparse matrix
length_data = length_matrix.data

# Step 2: Apply the power transformation to the non-zero elements
length_data_transformed = np.power(length_data, 0.5)

# Step 3: Reconstruct the sparse matrix with the transformed data
...
# # Convert to dense format for scaling
# length_matrix_dense = length_matrix.toarray()

# # Mean and standard deviation scaling
# length_mean = np.mean(length_matrix_dense[length_matrix_dense != 0])
# length_std = np.std(length_matrix_dense[length_matrix_dense != 0])
# length_matrix_dense = (length_matrix_dense - length_mean) / length_std

# # Convert back to sparse format
# length_matrix = sparse.csr_matrix(length_matrix_dense)

# length_matrix = np.log1p(length_matrix)  # log1p is log(1 + x), avoids log(0) issue


data_list = [length_matrix]
...
# for beta_init in np.linspace(-1, 0, 100):  # Grid search between -10 and 0
