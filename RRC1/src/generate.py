...
# Test with a single pair of origin and destination
orig_indices = [ori_node]
dest_indices = [des_node]

# Generate a single trajectory using the model
obs = model.generate_observations(origin_indices=orig_indices, dest_indices=dest_indices,
                                  num_obs_per_pair=1, iter_cap=2000, rng_seed=42)

print(f"Generated {len(obs)} trajectory")

# Convert node sequences back to edge sequences for saving
def convert_nodes_to_edges(node_path, index_to_node, edges):
    edge_list = []
    # Ignore the first element (des_node) and iterate over the rest
...
# Convert node paths back to edge paths
generated_traj_df['trajectory_edges'] = generated_traj_df['trajectory'].apply(
    lambda x: convert_nodes_to_edges(list(map(int, x.split('_'))), index_to_node, edges)
)

# Save the result to a CSV file
generated_traj_df.to_csv('generated_trajectory_single_test.csv', index=False)
print("Generated trajectory saved to 'generated_trajectory_single_test.csv'")